...
parser.add_argument('--arch_type', type=str, default='PEG',choices=['PEG','GAT','GAT_PE', 'GAT_PE_v2'])
...
for i in seed_list:
    ...
    if args.arch_type == 'PEG':
        model = Net(in_feats_dim = len(features[1]), pos_dim = args.PE_dim, hidden_dim = args.hidden_dim)
    elif args.arch_type == 'GAT':
        model = NetGAT(in_feats_dim = len(features[1]), pos_dim = args.PE_dim, hidden_dim = args.hidden_dim)
    elif args.arch_type == 'GAT_PE':
        model = NetGAT_PE(in_feats_dim = len(features[1]), pos_dim = args.PE_dim, hidden_dim = args.hidden_dim)
    elif args.arch_type == 'GAT_PE_v2':
        model = NetGAT_PE_v2(in_feats_dim = len(features[1]), pos_dim = args.PE_dim, hidden_dim = args.hidden_dim)
    ...
    if args.random_partition:
        ...
        results = train_model_plus(model, optimizer, x, edge_index,id_train_positive, id_train_negative,train_matrix, features, 
                    test_loader, val_loader, PE_dim = args.PE_dim, PE_method = args.PE_method, training_batch_size = args.batch_size, device = device, epochs = args.epochs)
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.weight_decay)
        results = train_model(model, optimizer, x, edge_index, train_loader, val_loader, test_loader, device = device, epochs = args.epochs)
