...
def train_model(model, optimizer, x, edge_index, train_loader, val_loader, test_loader, device, epochs):
    m = torch.nn.Sigmoid()
    loss_fct = torch.nn.BCELoss()
    train_loss_history = []
    val_loss_history = []
...
def train_model_plus(model, optimizer, x, edge_index, id_train_positive, id_train_negative,train_matrix, features, 
                val_loader, test_loader, PE_dim, PE_method, training_batch_size, device, epochs):
    m = torch.nn.Sigmoid()
    loss_fct = torch.nn.BCELoss()
    loss_history = []
    max_auc = 0
