...
from models import RGCN, RGCN_pos
...
def main(args):
    ...
    entity2id, relation2id, train_triplets, valid_triplets, test_triplets, train_matrix = load_data('./data/wn18')
    # print(len(np.where(train_matrix==1)[0]))
    ...
    if args.pos:
        model = RGCN_pos(len(entity2id), len(relation2id), num_bases=args.n_bases, dropout=args.dropout)
    else:
    ...
    checkpoint = torch.load('best_mrr_model_wn18.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pos", action='store_true', help='Use positional embedding')
    parser.add_argument("--pos_dim", type=int, default=100, help='dimension of positional embedding')
