...
def load_data(file_path):
    '''
        ...
        return:
            ...
            train_matrix: The adjacent matrix of train graph
            TODO: Currently the train_matrix is a pseudo-describe of the train graph(following the fashion of PEG), 
            virually there might be several relations (i.e. edges) between two nodes.
    '''
    ...
    train_triplets, train_matrix = read_triplets(os.path.join(file_path, 'train.txt'), entity2id, relation2id, matrix=True)
    ...
    return entity2id, relation2id, train_triplets, valid_triplets, test_triplets, train_matrix

def read_triplets(file_path, entity2id, relation2id, matrix=False):
    triplets = []
    if matrix:
        train_matrix = np.zeros([len(entity2id), len(entity2id)])
    with open(file_path) as f:
        for line in f:
            head, relation, tail = line.strip().split('\t')
            x, y = entity2id[head], entity2id[tail]
            triplets.append((x, relation2id[relation], y))
            if matrix:
                train_matrix[x, y] = 1

    if matrix:
        return np.array(triplets), train_matrix
    else:
